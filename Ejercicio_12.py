...
def simular_lanzamientos(n: int = 10000) :
    """
    Simula múltiples lanzamientos de dos dados y cuenta la frecuencia
    de cada suma posible.

    Args:
        n (int): Número de lanzamientos a simular (por defecto 10,000).

    Returns:
        dict[int, int]: Diccionario con la suma como clave
                        y la frecuencia como valor.
    """
    if n <= 0:
...
def mostrar_reporte(frecuencias) -> None:
    """
    Imprime un reporte de las frecuencias de cada suma.

    Args:
        frecuencias (dict[int, int]): Diccionario con las frecuencias.
    """
    print("\n--- Reporte de Frecuencias ---")
    for suma in range(2, 13):  # Todas las posibles sumas de 2 a 12
